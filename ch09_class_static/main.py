...
1. 클래스 변수 vs. 인스턴스 변수
    ...
    클래스 변수 : 보든 인스턴스가 동일한 값을 지니는 변수(Java에서는 정적 변수)
...
'''
객체명.클래스변수 를 통해서 클래스 변수에 접근이 가능하긴 하지만, 클래스 내부의 코드를 들여다보기 전까지는 country라는 변숙가 인스턴스 변수인지 클래스 변수인지 알 방법이 없다는 문제가 있습니다.
...
#     def trip(cls, traveling_site):      # 그 결과 첫 번째 매개변수가 self가 아니라 cls
...
2. man 과 woman 인스턴스가 생성되면 다음과 같은 메세지를 출력할 수 있도록 작성하시오.
김일이(가) 태어났습니다.
김이이(가) 태어났습니다.
...
5. man 인스턴스가 삭제되면 다음과 같은 메세지를 출력할 수 있도록 소명자를 정의하시오.
RIP 김일
'''
# class Person:
#     population = 0
...
'''
다음의 지시 사항을 읽고 가게의 매출을 구할 수 있는 Shop 클래스를 작성하시오.
...
class Shop:
    ...
    def sales(cls, name, count):
        ...
        if cost == 0:
            print(f'{name} 메뉴가 없습니다.')
            return

        print(f'{name}을(를) {count} 개 판매')
        cls.total += cost * count
