...
# python객체 만드는 방식이죠. 이전과 차이가 있다면 외부 모듈(파일)에서 가지고 온 클래스의 객체를 생성했다는 점입니다. Scanner scanner = new Scanner(System.in);과 같습니다.
...
while is_on:
    ...
    if choice == 'off':
        ...
        # 음료 이름을 입력 받은 시점부터의 프로세스를 떠올려서 코드를 작성하셔야만 합니다. 이때 고려해야할 것은 절차지향 방식으로 코딩했을 때의 과정과, 현재 참조 해야 하는 파이썬 모듈들의 매개변수 차이와 메서드 작동 방식 차이를 감안해서 코드를 쓸 필요가 있다는 점
        ...
        if coffee_maker.is_resource_sufficient(drink):
            ...
            # 모든 것이 true 일 때 '작동 중입니다'를 출력할 수 있도록 작성하시오.
